...
from meorphis_test_32 import Petstore, BaseModel, AsyncPetstore
from meorphis_test_32._response import (
    APIResponse,
    BaseAPIResponse,
    AsyncAPIResponse,
    BinaryAPIResponse,
...
from meorphis_test_32._streaming import Stream
from meorphis_test_32._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'meorphis_test_32._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: Petstore) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from meorphis_test_32 import BaseModel`",
    ):
